"""
generate_vehicle_master.py
===========================
AutoIntel AI — Dataset 3: Vehicle Master Dataset (Custom)

WHY THIS SCRIPT EXISTS
-----------------------
vehicle_maintenance_clean.csv (Member 1's output) describes SERVICE RECORDS
(mileage, last_service_date, reported_issues, etc.) but it does NOT contain
any vehicle identity info — no Vehicle_ID, no Brand, no Model, no Manufacturing
Year. It only has 3 identity-adjacent columns: vehicle_model (a *category*
like "Car"/"Truck", not a real model name), fuel_type, and engine_size.

The project workflow (Module 1 - Vehicle Selection) needs a separate
"vehicle_master.csv" that stores actual vehicle identity (Brand, Model,
Variant, Engine, Manufacturing Year) keyed by Vehicle_ID, so the Streamlit
app can let a user pick "Tata Nexon XZ+ (2021)" instead of an abstract row.

Since vehicle_maintenance_clean.csv never had Vehicle_IDs to begin with,
we CANNOT "reuse" existing IDs (there are none). Instead we:
  1. Build vehicle_master.csv containing ~200 realistic vehicle records,
     each tagged with the SAME vocabulary the maintenance dataset already
     uses (vehicle_model category, fuel_type, engine_size bucket). This
     shared vocabulary is what makes the two datasets "compatible".
  2. Generate a Vehicle_ID for every master record (VEH0001, VEH0002, ...).
  3. Back-fill a Vehicle_ID column into a COPY of the maintenance dataset by
     randomly assigning each service record to a master vehicle that has
     the exact same (vehicle_model, fuel_type, engine_size) signature.
     This is the standard approach for fleet/service datasets where many
     service records point back to a smaller pool of physical vehicles —
     it's also why the row count (50,000) so vastly exceeds a realistic
     fleet size (100-300 vehicles): each vehicle gets serviced many times
     over its life.

We do NOT touch the original vehicle_maintenance_clean.csv on disk — we
write the ID-augmented version to a new file so Member 1's original
deliverable stays untouched and auditable.
"""
